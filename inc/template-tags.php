...
if ( ! function_exists( 'Smitasmile_posted_on' ) ) :
	...
	function Smitasmile_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'Smitasmile' ),
...
if ( ! function_exists( 'Smitasmile_posted_by' ) ) :
	...
	function Smitasmile_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'Smitasmile' ),
	...
	function Smitasmile_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'Smitasmile' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'Smitasmile' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'Smitasmile' ),
					array(
						'span' => array(
							'class' => array(),
						),
